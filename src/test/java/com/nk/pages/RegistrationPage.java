...
public class RegistrationPage extends BasePageObject {
	...
	By yourCurrentEmploymentDetailsSection = By
			.xpath("//div[@id='lftcont']/div[contains(.,'Your Current Employment Details')]");
	By yourEducationBackgroundSection = By.xpath("//div[@id='lftcont']/div[contains(.,'Your Education Background')]");
	By uploadYourDetailedResumeSection = By
			.xpath("//div[@class='cls padLR10']/div[contains(.,'Upload your detailed resume')]");
	By mailerAndPrivacySettingsSection = By
			.xpath("//div[@class='cls padLR10']/div[contains(.,'Mailer and Privacy Settings')]");
	...
	By fastforwardEmailsCheckBox = By
			.xpath("//div[@class='fleft cls w260']/div[contains(.,'Naukri FastForward E-Mails')]");
	By fastforwardSMSCheckBox = By
			.xpath("//div[@class='fleft cls w260']/div[contains(.,'Naukri FastForward Calls/SMS ')]");
	By notificationsCheckBox = By
			.xpath("//div[@class='fright w265']/div[contains(.,'Important Notifications from Naukri.com ')]");
	By communicationCheckBox = By
			.xpath("//div[@class='fright w265']/div[contains(.,'Communication from Clients(Voice/Calls/SMS')]");
	...
	public void verifyRegistrationPageMessage() {
		...
		Assert.assertEquals(regMessage, NKConstants.MESSAGE_REGISTRATIONPAGE,
				"Expected Registration Page Message is not displayed");
	...
	/**
	 * This method will help us to verify Your Current Employment Details
	 * Section
	 * 
	 * @author Senthil
	 * @return flag
	...
	public void verifyEmailErrorDisplayed() {
		...
		Assert.assertEquals(emailErrorDisplayed, NKConstants.ERROR_EMAIL,
				"Email Error doesnot match with the expected error ");
	...
	public void verifyPasswordErrorDisplayed() {
		...
		Assert.assertEquals(passwordErrorDisplayed, NKConstants.ERROR_PASSWORD,
				"Password Error doesnot match with the expected error ");
	...
	public void verifyConfirmPasswordErrorDisplayed() {
		...
		Assert.assertEquals(cPwdError, NKConstants.ERROR_CONFIRMPASSWORD,
				"Expected Confirm Password Error doesnot match with the error displayed");
	}
	...
	public void verifyLocationDDErrorDisplayed() {
		...
		Assert.assertEquals(locError, NKConstants.ERROR_CURRENTLOCATION,
				"Expected Location Dropdown Error doesnot match with the displayed error");
	}
	...
	public void verifyMobileNumberErrorDisplayed(String errorExpectedmobilenumber) {
		...
		Assert.assertEquals(mobileErr, errorExpectedmobilenumber,
				"Expected Mobile number Error doesnot match with the displayed error");
	}
	...
	public void verifyNonSupportedFileErrorDisplayed() {
		...
		Assert.assertEquals(fileError, NKConstants.ERROR_INVALIDFILEPATH,
				"Expected Non-Supported File Error doesnot match wjth the displayed error");
	...
	public void verifyIAgreeErrorDisplayed() {
		...
		Assert.assertEquals(errorIAgree, NKConstants.ERROR_IAGREE,
				"Expected I Agree Error doesnot match with the error displayed");
	}
	...
	public void verifyRegistrationPageTitle() {
		...
		// Assert.assertEquals(pageTitle, NKConstants.PAGETITLE_REGISTRATION,
	...
	public void enterResumeText(String rText) throws Exception {
		...
		try {
			...
			throw new Exception("FAILED WHILE ENTERING TEXT IN RESUME TEXT AREA:::" + "\n enterResumeText(rText)"
					+ e.getLocalizedMessage());
		}
	}

	public void verifyUploadedFile() throws InterruptedException {
		verifyBrowseButton();
		Thread.sleep(2000);
		String docName = uiDriver.findElement(browseButton).getAttribute("value").trim();
		Assert.assertEquals(docName, NKConstants.UPLOADEDFILENAME,
				"The uploaded file names doesnot match with the displayed file name");
	}
